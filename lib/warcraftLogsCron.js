const { Character, Item, Presence, Enchant, Buff, InstanceStats } = require('../models/index')
const axios = require('axios')
const moment = require('moment')
const parser = require('fast-xml-parser')

const getItems = async () => {
  const zones = [{ id: 1000, name: 'mc' }, { id: 1001, name: 'onyxia' }, { id: 1002, name: 'bwl' }, { id: 1003, name: 'zg' }, { id: 1004, name: 'aq20' }, { id: 1005, name: 'aq40' }]
...
const getInstancesStats = async () => {
  console.log('Starting Getting InstanceStats')

  const zones = [{ id: 1000, name: 'mc' }, { id: 1001, name: 'onyxia' }, { id: 1002, name: 'bwl' }, {
    id: 1003,
    name: 'zg'
  }, { id: 1004, name: 'aq20' }, { id: 1005, name: 'aq40' }]
  const partitions = [1, 2, 3, 4]

  const characters = await Character.find({ main: true }).populate('userId')
  for (const character of characters) {
    const finalObject = { all: 0 }
    ...
    if (character.userId && character.userId.roles && (character.userId.roles.includes('member'))) {
      try {
        for (const zone of zones) {
          ...
          for (const partition of partitions) {
            console.log(`Loading data for user instance stats ${character.name} from zone ${zone.name} and partition ${partition}`)
            const result = await axios.get(encodeURI(`https://classic.warcraftlogs.com:443/v1/parses/character/${character.name}/Sulfuron/EU?zone=${zone.id}&partition=${partition}&api_key=${process.env.WARCRAFTLOGS_API_KEY}`))
            if (finalObject[zone.name] === undefined) {
              finalObject[zone.name] = 0
            }

            for (const report of result.data) {
              if (!reports.includes(`${report.reportID}${report.encounterID}`)) reports.push(`${report.reportID}${report.encounterID}`)
            }

            await sleep(200)
          }

          finalObject[zone.name] = reports.length
          finalObject.all += reports.length
        }
        await InstanceStats.findOneAndUpdate({ characterId: character._id }, { stats: finalObject }, { new: true, upsert: true })
      } catch (e) {
        console.log(e)
      }
      await sleep(1000)
  ...
  console.log('End Getting Instance Stats')
}
